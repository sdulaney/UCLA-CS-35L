...
cat in | python shuf.py >out || fail=1
c=$(wc -l < out)
...
python shuf.py in1 >out 2>out.2
test $? -eq 2 || fail=1
...
python shuf.py in in1 >out 2>out.2
test $? -eq 2 || fail=1
...
python shuf.py -i 1-100 > out || fail=1
cmp in out > /dev/null && { fail=1; echo "not random?" 1>&2; }
sort -n out > out1
cmp in out1 > /dev/null || { fail=1; echo "not a permutation" 1>&2; }
...
for ARG in '1' 'A' '1-' '1-A' '3-1'; do
    { python shuf.py -i$ARG > out 2> out.2 || test $? -ne 2; } &&
    { fail=1; echo "shuf did not detect erroneous -i$ARG usage.">&2 ; }
done

# -i cannot be used with any non-option arguments
python shuf.py -i 1-100 - > out 2> out.2
test $? -eq 2 || fail=1
python shuf.py -i 1-100 in > out 2> out.2
test $? -eq 2 || fail=1
touch in2
python shuf.py -i 1-100 in in2 > out 2> out.2
test $? -eq 2 || fail=1
rm in2
...
python shuf.py -i 1-100 -n 9 > out || fail=1
c=$(wc -l < out)
test "$c" -eq 9 || { fail=1; echo "Option -n failed">&2 ; }

# Test invalid arguments for option -n
python shuf.py in -n A >out 2>out.2
test $? -eq 2 || fail=1
python shuf.py in -n -1 >out 2>out.2
test $? -eq 2 || fail=1

##### Option --repeat (-r)

# Exercise shuf's -r option.
python shuf.py -i 1-10 -n 100 -r > out || fail=1
c=$(wc -l < out)
...
python shuf.py --help > out || fail=1
...
# TODO: option -i with missing/invalid mandatory argument

# TODO: option -n with missing/invalid mandatory argument

# Exercise shuf's --input-range option.
#python shuf.py --input-range=1-100 > out || fail=1
...
#python shuf.py -i 1-100 --head-count=9 > out || fail=1
#c=$(wc -l < out)
#test "$c" -eq 9 || { fail=1; echo "Option --head-count failed">&2 ; }


# Exercise shuf's --repeat option.
#python shuf.py -i 1-10 -n 100 --repeat  > out || fail=1
#c=$(wc -l < out)
...
#cat in | python shuf.py -i 0-9 >out || fail=1
#c=$(wc -l < out)
...
#cat in | python shuf.py -r -n 1000 >out || fail=1
#c=$(wc -l < out)
...
#cat in | python shuf.py -r -n 1000 -i 0-9 >out || fail=1
#c=$(sort -u < out)
...
#cat in | python shuf.py - -i 0-9 >out
#test $? -eq 2 || fail=1
...
#  python shuf.py -n0 unreadable || fail=1
#  { python shuf.py -n1 unreadable || test $? -ne 2; } && fail=1
#fi

# Multiple -n is accepted, should use the smallest value
#python shuf.py -n10 -i0-9 -n3 -n20 > exp
#c=$(wc -l < exp)
...
#{ python shuf.py -nA || test $? -ne 2; } &&
#  { fail=1; echo "shuf did not detect erroneous -n usage.">&2 ; }
# Test multiple -i
#{ python shuf.py -i0-9 -n10 -i8-90 || test $? -ne 2; } &&
...
#python shuf.py --rep -i0-9 -n1000 > exp
#c=$(wc -l < exp)
...
#python shuf.py --rep -i222-233 -n2000 > exp
#c=$(sort -nu exp | paste -s -d ' ')
...
#python shuf.py --rep -i0-9 -n0 > exp
...
#python shuf.py /dev/null <&- >out || fail=1
