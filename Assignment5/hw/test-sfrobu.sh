...
test=1
printf '*~BO *{_CIA *hXE]D *LER #@_GZY #E\\OX #^BO #FKPS #NEM\4' | ./sfrobu | od -ta > out
...
#test=7
#printf '*~BO *{_CIA *hXE]D *LER #@_GZY #E\\OX #^BO #FKPS #NEM\4' | ./sfrobu >&- > out.1 2> out.2
#test $? -eq 1 || echo "Test $test: wrong exit code"
#cmp -s out.2 /dev/null && echo "Test $test: STDERR empty"
#cmp out.1 /dev/null || echo "Test $test: STDOUT not empty"
...
test=8
printf '*~BO *{_CIA *hXE]D *LER #@_GZY     #E\\OX #^BO #FKPS #NEM\4' | ./sfrobu > out
test $? -eq 0 || echo "Test $test: wrong exit code"
...
test=10
cat /proc/self/status | ./sfrobu > out
test $? -eq 0 || echo "Test $test: wrong exit code"
#printf '     ' > expected_output
#cmp expected_output out > /dev/null || echo "Test $test: wrong STDOUT"
...
#test=11
#cat /dev/urandom | head -c 30000000000 | ./sfrobu > out.1 2> out.2
#test $? -eq 1 || echo "Test $test: wrong exit code"
#cmp -s out.2 /dev/null && echo "Test $test: STDERR empty"
#cmp out.1 /dev/null || echo "Test $test: STDOUT not empty"
